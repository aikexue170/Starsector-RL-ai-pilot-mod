...
from abandoned_code.dqn_agent_LSTM import DQNAgent  # 注意这里改为LSTM版本
...
RAY_MAX_DISTANCE = 1  # 在游戏端已经被归一化了

# 添加动作掩码来限制战术系统使用频率
TACTICAL_COOLDOWN_THRESHOLD = 2.0  # 冷却时间阈值

# LSTM特定参数
SEQUENCE_LENGTH = 4
MODEL_TYPE = "dqn_lstm"  # 改为使用LSTM模型


class StarSectorEnv:
    """简化版游戏环境 - LSTM版本"""

    def __init__(self, max_steps=1000, sequence_length=SEQUENCE_LENGTH):
        # Socket服务器初始化
        self.host = '127.0.0.1'
        self.port = 65432
        ...
        self.state_sequence = deque(maxlen=sequence_length)  # 维护状态序列
    ...
    def reset(self):
        ...
        for _ in range(self.sequence_length):
            self.state_sequence.append(initial_state)
        # 设置新目标
        self._reset_target()
    ...
    def step(self, action):
        ...
        return self._get_current_sequence(), reward, done, {}

    def _get_current_sequence(self):
        """获取当前状态序列"""
        if len(self.state_sequence) < self.sequence_length:
            # 如果序列不足，用当前状态填充
            current_state = self.state_sequence[-1] if self.state_sequence else self._get_state()
...
def main():
    # 创建环境
    env = StarSectorEnv(max_steps=MAX_STEPS, sequence_length=SEQUENCE_LENGTH)
    state_dim = 30
    action_dim = 8
    ...
    print(f"序列长度: {SEQUENCE_LENGTH}, 模型类型: {MODEL_TYPE}")
    ...
    try:
        for episode in range(EPISODES):
            ...
            total_reward = 0
            success = False
            tactical_cooldown_remaining = 0  # 初始化冷却时间
            tactical_usage_count = 0  # 统计战术系统使用次数

            for step in range(MAX_STEPS):
                # 更新冷却时间
                if tactical_cooldown_remaining > 0:
                    tactical_cooldown_remaining -= 0.1  # 假设每步0.1秒

                # 选择动作 - 传入战术冷却信息
                action = agent.select_action(
                    state_sequence,
                    tactical_cooldown=tactical_cooldown_remaining
                )

                # 如果选择了战术动作，重置冷却时间
                if action == 7:  # 假设7是战术动作
                    tactical_cooldown_remaining = 5.0  # 5秒冷却
                    tactical_usage_count += 1

                # 执行动作
                next_state_sequence, reward, done, _ = env.step(action)

                # 存储经验
                current_state = state_sequence[-1]  # 序列的最后一个状态
                next_state = next_state_sequence[-1]  # 下一个序列的最后一个状态

                agent.push_memory(current_state, action, reward, next_state, done)

                # 训练 - 降低训练频率以加快速度
                if len(agent.memory) > BATCH_SIZE and step % 2 == 0:  # 每2步训练一次
                    loss = agent.train()
            ...
            # 计算战术系统使用率
            tactical_usage_rate = tactical_usage_count / min(MAX_STEPS, step + 1) * 100
            ...
            if episode % 10 == 0:
                ...
                print(f"Episode {episode}/{EPISODES} | "
                      ...
                      f"Tactical Usage: {tactical_usage_rate:.1f}% | "
            ...
            if episode % 100 == 0:
                if not os.path.exists("models"):
                    os.makedirs("models")
                save_path = f"models/dqn_lstm_model_episode_{episode}.pth"
        ...
        # 保存最终模型
        agent.save("dqn_lstm_final_model.pth")
        print("LSTM训练完成，最终模型已保存为 dqn_lstm_final_model.pth")
